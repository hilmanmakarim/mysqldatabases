...
-- decimal(3,2) : 3 digits numbers and 2 digits behind comma
...
create table orders(
    ...
    constraint FK_CustomertId foreign key (customer_id) references customers(id) on delete set null on update restrict
);
desc orders;
...
-- menambah kolom baru(kolom location ditentukan)
alter table customers add column address varchar(255) after gender;
desc customers;

-- menghapus column
alter table products drop column rigin;
desc products;
...
-- isert data
-- insert into <table_name> (<list of column>) values (<list of value>)
insert into products (name, price, origin) 
values ('Ice Coffee',3.3,'Sri Lanka'),('Cappucino',3.5,'Sumatra'),('latte',3.4,'Brazil'),('Americano',3.5,'Java'),('Flat White',3.6,'Kenya'),('Filtered COffee', 4.0, 'Vietnam'),('Macchiato', 4.5,'Bali'),('Green Tea Cappucino',4.8,'Guatemala');

insert into products (name, price, origin) values ('Macchiato', 4.5,'Bali');
insert into products (name, price, origin) values ('Black Tea ', 4.1,'Bali');

-- melihat data
select * from products;

-- update data
update products set origin = 'Sri Lanka' where id = 1;
update products set price = 3.3 where id = 1;
update products set name = 'Ice Coffe' where id = 1;

set sql_safe_updates = 0;
update products set price=4.9,origin='Guatemala',name= 'Green Tea Cappucino' where id = 0;

-- delete product
delete from products where price = 4.10;
