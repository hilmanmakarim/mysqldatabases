...
-- TAMPILKAN NAMA DEPAN, BELAKANG, DAN NAMA FILM UNTUK 5 CUSTOMER PERTAMA YANG MENONTON DI RUANGAN 'KUBRICK'
SELECT c.first_name, c.last_name, f.name, r.name FROM bookings b
JOIN (screenings s, customers c) ON c.id=b.customer_id AND s.id = b.screening_id JOIN (films f, rooms r) ON f.id= s.film_id AND r.id=s.room_id
WHERE r.name ='Kubrick' ORDER BY b.id LIMIT 5;

-- TAMPILKAN BOOKING ID, EMAIL, NAMA FILM DAN DURASI UNTUK 3 FILM TERAKHIR YANG DIPESAN CUSTOMEN DENGAN EMAIL SHOOVER@GMAIL.COM
SELECT b.id, c.email, f.name, f.length_min FROM bookings b
JOIN (customers c, screenings s, films f) ON c.id=b.customer_id AND b.screening_id= s.id AND s.film_id= f.id
WHERE c.email = 'shoover@gmail.com' ORDER BY b.id DESC LIMIT 3;

-- TAMPILKAN NAMA FILM DAN DURASI SECARA UNIQUE UNTUK FILM YANG DIPUTAR DI RUANGAN CHAPLIN
SELECT DISTINCT f.name, f.length_min, r.name FROM screenings s
JOIN (films f, rooms r) ON f.id = s.film_id AND r.id=s.room_id
JOIN (films f, rooms r) ON f.id = s.film_id AND r.id=s.room_id
WHERE r.name= 'Chaplin';

-- TAMPILKAN EMAIL, NOMOR, BARIS BANGKU, UNTUK BANGKU APA SAJA YANG DIPESAN OLEH USER DENGAN EMAIL JP@GMAIL.commit
SELECT c.email, ss.number, ss.row_num FROM bookings b
JOIN (customers c, reserved_seat rs, seats ss) ON c.id = b.customer_id AND b.id=rs.booking_id AND ss.id=rs.seat_id
WHERE c.email= 'jp@gmail.com'; 

-- TAMPILKAN FIRST NAME DAN LAST NAME UNTUK CUSTOMER YANG MEMESAN FILM 'JIGSAW' DIURUTKAN BERDASARKAN WAKTU TAYANG
SELECT c.first_name, c.last_name, s.start_time, ss.number, ss.row_num,f.name FROM bookings b
JOIN (customers c, screenings s, films f, reserved_seat rs, seats ss) ON c.id=b.customer_id AND s.id=b.screening_id AND f.id=s.film_id AND b.id= rs.booking_id AND ss.id = rs.seat_id
...
-- BANYAK CUSTOMER YANG TIDAK MEMILIKI NAMA DEPAN
SELECT count(*) FROM customers WHERE first_name is NULL;

-- SUM
-- Banyaknya bangku dalam satu bioskop (gabungan 3 ruangan)
SELECT sum(no_seats) FROM rooms;

-- MAX
-- JUMLAH BANGKU TERBANYAK DALAM SUATU RUANGAN
...
-- banyaknya booking yang dilakukan oleh customer
-- NAMA FILM, WAKTU TAYANG, NAMA DEPAN, NAMA BELAKANG
select f.name, s.start_time, count(f.name) FROM bookings b
JOIN (screenings s, films f) ON s.id =b.screening_id AND f.id=s.film_id
GROUP BY f.name, s.start_time
ORDER BY s.start_time;

-- BEBRAPA BANYAK JADWAL (SCREENING) UNTUK FILM 'BLADE RUNNER 20499' PADA BULAN OKTOBER
SELECT f.name, count(*) FROM films f
JOIN screenings s ON f.id=s.film_id
WHERE s.start_time LIKE '2017-10-%' AND f.name = 'Blade Runner 2049';

-- EXERCISE
-- BERAPA BANYAK BOOKING UNTUK SETIAP FILM DI BULAN OKTOBEBR TAMPILKAN NAMA FILM, BANYAK BOOKING

SELECT f.name, count(f.name) FROM bookings b
JOIN (screenings s, films f) ON s.id =b.screening_id AND f.id=s.film_id
WHERE s.start_time LIKE '2017-10-%'
GROUP BY f.name
ORDER BY f.name;
 
-- tampilkan nama film, banyak jadwal (screening) per film order y banaknya jadwal
...
-- TAMPILKAN NAMA DEPAN, NAMA BELAKANG, DAN total booking
-- UNTUK 3 CUSTOMER YANG MELAKUKAN BOOKING terbanyak

SELECT c.first_name, c.last_name, count(*) FROM customers c
JOIN (bookings b, screenings s) ON c.id=b.customer_id AND s.id = b.screening_id JOIN (films f, rooms r) ON f.id= s.film_id AND r.id=s.room_id
group by c.first_name, c.last_name ORDER BY count(*) DESC LIMIT 3;

-- TAMPILKAN NAMA DEPAN, NAMA BELAKANG, TOTAL BANGKU YAG PERNAH DIPESAN UNTUK 3 ORANG DG TOTAL BANGKU YANG PERNAH DIPESAN
SELECT c.first_name, c.last_name, count(*) FROM reserved_seat rs
JOIN (customers c, bookings b, seats s) ON c.id=b.customer_id AND b.id = rs.booking_id AND s.id = rs.seat_id
GROUP BY c.first_name, c.last_name order by count(*) DESC LIMIT 3;
...
-- HAVING
-- TAMPILKAN NAMA DEPAN, NAMA BELAKANG, DAN total booking
-- UNTUK TOTAL BOOKING > 9
SELECT c.first_name, c.last_name, count(*) FROM bookings b
JOIN customers c ON c.id =b.customer_id
GROUP BY c.first_name, c.last_name
HAVING count(*) > 9
ORDER BY count(*) DESC;

-- TAMPILKAN NAMA DEPAN, NAMA BELAKANG, TOTAL BANGKU YAG PERNAH DIPESAN
-- UNTUK TOTAL BANGKU YANG LEBIH DARI 20
SELECT c.first_name, c.last_name, count(*) as total_seats FROM bookings b
JOIN (customers c, reserved_seat rs, seats s) ON c.id=b.customer_id AND b.id = rs.booking_id AND s.id = rs.seat_id
GROUP BY c.first_name, c.last_name
having total_seats > 20
order by count(b.id) DESC;

-- SUB QUERY
-- mencari film yang memiliki durasi film lebih dari rata-rata
select name, length_min from films f
WHERE length_min > (select AVG(length_min) from films);

-- mencari ruangan dengan jumlah bangku di atas rata-rata
...
SELECT dayofmonth('2018-06-05 07:32:45') as dayOfmonth;
SELECT month ('2018-06-05 07:32:45') as monthNow;
SELECT year ('2018-06-05 07:32:45') as yearNow;

-- SCREENING ANG BERJALAN DI BULAN SEPULUH
SELECT * from screenings where month (start_time) = 10;

-- SCREENING YAYNG BERJALAN DI TAHUN 2017
SELECT * FROM screenings WHERE YEAR (start_time) =2017;

-- screening yang bberjalan di tanggal 8, 14 dan 22
SELECT * FROM screenings where dayofmonth(start_time) in (8,14,22);


-- exercise

-- film apa saja yang memiliki durasi lebih dari 2 jam
select name, length_min from films f
WHERE length_min > 120 ORDER BY length_min DESC;


-- film apa saja yang memiliki jadwal tampil paling banyak di bulan oktober

SELECT f.name, start_time, count(*) FROM screenings s
JOIN films f ON f.id=s.film_id
GROUP BY f.name
HAVING start_time LIKE '2017-10-%'
ORDER BY count(*) DESC;

-- cara kedua
SELECT f.name, count(s.film_id) as banyaknya FROM screenings s
JOIN films f ON f.id=s.film_id
WHERE month(s.start_time)='10'
GROUP BY f.name
ORDER BY banyaknya DESC;

-- berapa banyak booking yang dimiliki oleh film 'jigsaw' di bulan oktober

SELECT f.name, count(*) FROM bookings b
JOIN (screenings s, films f) ON s.id = b.screening_id AND f.id=s.film_id
WHERE s.start_time LIKE '2017-10-%' AND f.name = 'Jigsaw'; 

-- CARA KEDUA
SELECT f.name, count(s.id) AS total_booking FROM bookings b
JOIN (screenings s, films f) ON s.id = b.screening_id AND f.id=s.film_id
where f.name ='Jigsaw' AND MONTH(s.start_time) ='10';

-- film yang paling banyak tayang di ruangan 'chaplin'

SELECT f.name, r.name, count(*) FROM rooms r
JOIN (screenings s, films f) ON r.id = s.room_id AND f.id = s.film_id
GROUP BY f.name HAVING r.name ='Chaplin'
ORDER BY count(*);

SELECT f.name, count(s.id) AS showings FROM screenings s
JOIN (films f, rooms r) ON f.id = s.film_id AND r.id=s.room_id
where r.name ='Chaplin' group by f.name order by showings desc limit 1;

-- Berapa banyak customer yang melakukan booking di bulan oktober

select count(distinct(c.id)) FROM customers c
JOIN (screenings s, bookings b) ON s.id = b.screening_id AND c.id = b.customer_id;
